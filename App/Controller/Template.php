...
class Template extends BaseController
{
    ...
    function get($f3)
    {
        ...
        $test->expect(
            $tpl->token($expr = '@foo->zip(@bar,\'qux\',123,[\'a\'=>\'hello\'])') ==
            ($eval = '$foo->zip($bar,\'qux\',123,[\'a\'=>\'hello\'])'),
            $expr.': '.$eval,
        );
        $test->expect(
        ...
        $test->expect(
            ($x = preg_replace('/(\s)*/', '', trim($tpl->render('templates/test5.4.htm')))) ===
            '1010',
        ...
        $test->expect(
            isset($lines[6]) && $lines[6] == $f3->stringify(
                ['@attrib' => ['bar' => 'baz', 'baz' => '{{ @abc }}'], 'multiple params switched'],
            ),
        ...
        $test->expect(
            isset($lines[8]) && $lines[8] == $f3->stringify(
                ['@attrib' => ['{{ @param }}'], 'tag with inline token'],
            ),
            'Custom tag with inline token',
        );
        $test->expect(
            isset($lines[9]) && $lines[9] == $f3->stringify(
                ['@attrib' => ['bar' => 'test10', '{{ @param }}'], 'param, inline token'],
            ),
        ...
        $test->expect(
            isset($lines[12]) && $lines[12] == $f3->stringify(
                ['@attrib' => ['bar' => 'test13'], 'simple tag'],
            ),
            'Custom tag (simple)',
        );
        $test->expect(
            isset($lines[13]) && $lines[13] == $f3->stringify(
                ['@attrib' => ['bar' => 'test14'], 'this {{ @token }} should NOT get resolved'],
            ),
            'Custom tag with inner HTML containing template token',
        );
        $test->expect(
            isset($lines[14]) && $lines[14] == $f3->stringify(
                ['@attrib' => ['bar' => 'test15', 'baz' => 'abc'], 'multi-line start tag'],
            ),
        ...
        $res = [];
        $f3->set('ex', function ($val) use (&$res) {
            $res[] = $val;
        });
        ...
        $tpl->filter('pick', '\App\Controller\Helper::instance()->pick');
        $test->expect(
            $tpl->filter('pick') == '\App\Controller\Helper::instance()->pick',
        ...
        $test->expect(
            ...
            ($eval = '\App\Controller\Helper::instance()->pick($foo)'),
            'Resolve custom filter: {{'.$expr.'}} > '.$eval,
        );
        $test->expect(
            $tpl->token($expr = '@foo, \'bar\' | pick') ==
            ($eval = '\App\Controller\Helper::instance()->pick($foo, \'bar\')'),
            'Resolve filter with arguments: {{'.$expr.'}} > '.$eval,
        );
        $test->expect(
            $tpl->token($expr = '@foo, \'bar|baz\' | pick') ==
            ($eval = '\App\Controller\Helper::instance()->pick($foo, \'bar|baz\')'),
        ...
        $test->expect(
            ...
            ($eval = '$this->esc(\App\Controller\Helper::instance()->pick($foo))'),
            'Multiple filter: {{'.$expr.'}} > '.$eval,
        );
        $test->expect(
            $tpl->token($expr = '@foo, @bar | pick, esc') ==
            ($eval = '$this->esc(\App\Controller\Helper::instance()->pick($foo, $bar))'),
        ...
        $tpl->filter('json_encode', '\App\Controller\Helper::instance()->json');
        $test->expect(
            $tpl->filter('json_encode') == '\App\Controller\Helper::instance()->json',
        ...
        foreach (
            glob(
                $f3->get('TEMP').
                $f3->hash($f3->get('ROOT').$f3->get('BASE')).'.*.php',
            ) as $file
        )
            unlink($file);
