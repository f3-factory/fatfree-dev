...
class Router extends BaseController
{
    ...
    public function get(Base $f3): void
    {
        ...
        $test->expect(
            $f3->alias('complex', 'format=20x20,*=[foo/bar,baz.gif]') ==
            '/resize/20x20/foo/bar/sep/baz.gif' &&
            $f3->alias('complex', 'format=20x20,*=[foo,bar]', ['x' => 123, 'y' => ['z' => 2]]) ==
            '/resize/20x20/foo/sep/bar?x=123&y%5Bz%5D=2',
            'Alias() function',
        );
        $f3->reroute('@hello');
        $rr1 = $f3->get('reroute');
        ...
        $f3->route(
            'GET|POST|PUT @grub:/food/@id/@quantity',
            function ($f3, $args) {
                $f3->set('id', $args['id']);
        ...
        $test->expect(
            $f3->GET == ['a' => 1] && $f3->POST == ['b' => 2] &&
            $f3->REQUEST == ['a' => 1, 'b' => 2] && $f3->get('BODY') == 'c=3',
        ...
        $test->expect(
            in_array('Access-Control-Allow-Origin: *', $headerlist),
            'CORS Preflight Origin test',
        );
        $test->expect(
            in_array('Access-Control-Allow-Methods: OPTIONS,GET,POST', $headerlist),
            'CORS Preflight Methods',
        );
        $test->expect(
            in_array('Access-Control-Allow-Credentials: true', $headerlist),
            'CORS Preflight Credentials',
        );
        $test->expect(
            in_array('Access-Control-Max-Age: 60', $headerlist),
        ...
        $test->expect(
            $out === 'cors' && in_array('Access-Control-Allow-Origin: *', $headerlist),
            'CORS Ajax Route test',
        );
        $f3->header_remove();
        ...
        $test->expect(
            in_array('Access-Control-Expose-Headers: X-Version,Foo', $headerlist),
            'CORS Request Expose Headers',
        );
        $f3->header_remove();
