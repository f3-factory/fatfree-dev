        ...
        foreach ($dbs as $db_key => $pdo_ext) {
            ...
            if ($loaded) {
                switch ($db_key) {
                    ...
                    case 'pgsql':
                        $db = new \F3\DB\SQL(
                            'pgsql:host=f3-pgsql;dbname=fatfree',
                            'fatfree',
                            'fatfree',
                        );
                        break;
                    case 'sqlsrv':
                        $db = new \F3\DB\SQL(
                            'sqlsrv:SERVER=f3-mssql;Encrypt=true;TrustServerCertificate=true;',
                            'sa',
                            'FatFree-r00t!',
                        );
                        break;
                ...
                if ($engine == 'sqlsrv') {
                    ...
                    unset($db);
                    $db = new \F3\DB\SQL(
                        'sqlsrv:SERVER=f3-mssql;Database=fatfree;Encrypt=true;TrustServerCertificate=true;',
                        'sa',
                        'FatFree-r00t!',
                    );
                }
                ...
                $ticket = new \F3\DB\SQL\Mapper($db, 'tickets', null, 0);
                ...
                $adhocTicket = new \F3\DB\SQL\Mapper($db, 'tickets', ['title'], 0);
                ...
                if ($engine != 'pgsql') { // PostgreSQL not supported (yet)
                    ...
                    $before = $after = '';
                    if (preg_match(
                        '/^Set-Cookie: '.session_name().'=(\w+)/m',
                        implode(PHP_EOL, array_reverse(headers_list() ?: $f3->RESPONSE_HEADERS)),
                        $m,
                    ))
                        $before = $m[1];
                    $f3->clear('SESSION');
                    ...
                    $test->expect(
                        ...
                        'Session destroyed and cookie expired:'.$before.'|'.$after.'|'.$sid.'|'.
                        empty($f3->COOKIE[session_name()]).'|'.empty($f3->SESSION),
                    );
                }
                $ticket->erase('');
                $test->expect(
