...
class Hive extends BaseController {
	...
	function get($f3) {
		...
		$f3->set('i',123);
		$test->expect(
			$f3->get('i')===123 && $f3->i===123,
			'Value assigned and retrieved'
		);
		$f3->clear('i');
		$test->expect(
			!$f3->exists('i') && empty($f3->i),
			'Value cleared and verified'
		);
		$f3->set('f',3.14);
		$test->expect(
			$f3->get('f')===3.14 && $f3->get('f')===$f3->f &&
			$f3->f===$f3->get('f'),
			'Float value'
		);
		$f3->set('b',TRUE);
		$test->expect(
			$f3->get('b')===TRUE && $f3->get('b')===$f3->b &&
			$f3->b===$f3->get('b'),
			'Boolean value'
		);
		$f3->set('e',6.78e-3);
		$test->expect(
			$f3->get('e')===6.78e-3 && $f3->get('e')===$f3->e &&
			$f3->e===$f3->get('e'),
			'Scientific notation'
		);
		...
		$test->expect(
			$f3->exists('a') && $f3->exists('a->hello',$hello) &&
				$hello=='world' && isset($f3->a) && isset($f3->a->hello),
			'Existence confirmed'
		);
		$f3->set('a->z.x','foo');
		$test->expect(
		...
		$f3->set('i.j.k','foo');
		$test->expect(
			is_array($f3->get('i')) && is_array($f3->i) &&
			is_array($f3->get('i.j')) && is_array($f3->i['j']) &&
			$f3->get('i.j.k')=='foo' && $f3->i['j']['k']=='foo',
			'Modified array'
		);
		$test->expect(
		...
		$x=&$f3->ref('x');
		$test->expect(
			$x=='open sesame!' && $f3->get('x')=='open sesame!' && $f3->x=='open sesame!',
			'Reference to variable'
		);
		$x=123;
		$test->expect(
			$x===123 && $f3->get('x')===123 && $f3->x===123,
			'Indirect assignment'
		);
		...
		$f3->copy('x','y');
		$test->expect(
			$f3->get('x')===$f3->get('y') && $f3->x===$f3->y,
			'Copy variable'
		);
		$f3->concat('y',' bar');
		$test->expect(
			$f3->get('y')=='foo bar' && $f3->y=='foo bar',
