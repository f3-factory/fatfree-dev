...
error_reporting(
    (E_ALL) & ~(E_NOTICE | E_USER_NOTICE | E_WARNING | E_USER_WARNING),
);
...
$ws = new F3\CLI\WS('tcp://0.0.0.0:9000');
...
$ws->on('start', function ($server) use ($fw) {
    ...
    if ($op == F3\CLI\WS::Text && $data) {
        $in = json_decode($data, true);
        $agent->send(F3\CLI\WS::Text, json_encode($in));
